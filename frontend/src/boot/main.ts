...
export default boot(async ({ app }) => {
  ...
  socketIo.on('connect', () => {
    socketIo.on('receive-message', async (msg) => {
      console.log(msg);
    });
  });
...
>>>>>>> b1af53fb6a113a38b70d7dacbbbccc67d6a27777
  ...
  const registerAccountGateway = new RegisterAccountGatewayHttp(
    axiosAdapter,
    baseUrl
  );
  const registerAccountAction = new RegisterAccountAction(
    registerAccountGateway
  );
  app.provide('registerAccountAction', registerAccountAction);

  const verifyAccountGateway = new VerifyAccountGatewayHttp(
    axiosAdapter,
    baseUrl
  );
  ...
  const recoverPasswordGateway = new RecoverPasswordGatewayHttp(
    axiosAdapter,
    baseUrl
  );
  const recoverPasswordAction = new RecoverPasswordAction(
    recoverPasswordGateway
  );
